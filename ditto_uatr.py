"""
ditto_uatr.py — Ditto for UATR (ShipsEar)
==========================================

Ditto (Li et al., ICML 2021) maintains TWO models per client:
  - w  : shared global model (updated via standard FedAvg aggregation)
  - v_i: personalized model  (updated locally, regularized toward w)

Local objective for personalized model:
    min_v  L_i(v) + (lambda/2) * ||v - w||^2

The global model w is trained exactly as FedAvg.
The personalized model v_i is trained after receiving the updated w,
using the proximal term to stay close to the global model.

Evaluation uses the PERSONALIZED model v_i (not the global w).
This is the key difference from FedProx — FedProx improves the
global model, Ditto learns per-client models that adapt from it.

lambda is searched via grid: {0.01, 0.1, 0.5, 1.0, 2.0}
...
Usage:
  python ditto_uatr.py --tune
  python ditto_uatr.py --lam 0.1
  python ditto_uatr.py --lam 0.1 --alpha 1.0 --clients 5
"""

import os, sys, time, argparse, json, copy
import numpy as np
import pandas as pd
import torch
...
ALGO        = "ditto"
RESULTS_DIR = "./ditto_results"
PLOTS_DIR   = "./ditto_plots"
LOGS_DIR    = "./ditto_logs"
...
# ─────────────────────────────────────────
# Plot helpers (ditto-specific folders)
# ─────────────────────────────────────────
...
def plot_combined(mean_logs, lam):
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle("Ditto — All Configurations (lambda=" + str(lam) + ")",
    ...
    for key, df in mean_logs.items():
        ...
        axes[1].plot(df["round"], df["f1"],
                     color=color, linestyle=ls, linewidth=2, label=label)
    axes[0].set_title("Accuracy vs Round (personalized model)")
    axes[0].set_xlabel("Round"); axes[0].set_ylabel("Accuracy (%)")
    axes[1].set_title("Macro F1 vs Round (personalized model)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "ditto_combined_accuracy_f1.png"),
                dpi=150, bbox_inches="tight")
    plt.close()
    print("  Combined: ditto_plots/ditto_combined_accuracy_f1.png")


def plot_summary_bar(rows, lam):
    ...
    ax.set_ylabel("Score")
    ax.set_title("Ditto — Best Accuracy & F1 (lambda=" + str(lam) + ")",
                 fontweight="bold")
    ...
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, "ditto_summary_bar.png"),
                dpi=150, bbox_inches="tight")
    plt.close()
    print("  Summary bar: ditto_plots/ditto_summary_bar.png")

# ─────────────────────────────────────────
# Personal model store — persists across rounds
# Flower creates a new client instance every round so we must
# store personal models outside the client object
# ─────────────────────────────────────────
_personal_models: dict = {}   # client_id -> state_dict
...
class DittoClient(fl.client.NumPyClient):
    """
    Ditto maintains two models:
      self.model    — global model w  (sent to/from server, trained as FedAvg)
      self.personal — personal model v_i (kept locally, never sent to server)

    IMPORTANT: Flower creates a new client instance every round.
    Personal models are stored in _personal_models dict keyed by client_id
    so they persist across rounds.

    fit() does:
      1. Update global model w via standard local SGD (same as FedAvg)
      2. Update personal model v_i with proximal term toward w:
             v_i = v_i - lr * (grad_L(v_i) + lambda * (v_i - w))

    evaluate() uses the PERSONAL model v_i.
    """
    def __init__(self, client_id: int, data_dir: str, lam: float):
        self.client_id = client_id
        self.model     = get_model().to(device)   # global model
        self.personal  = get_model().to(device)   # personal model

        self.criterion    = nn.CrossEntropyLoss()
        self.opt_global   = optim.Adam(
            filter(lambda p: p.requires_grad, self.model.parameters()),
            lr=LR, weight_decay=WEIGHT_DECAY,
        )
        self.opt_personal = optim.Adam(
            filter(lambda p: p.requires_grad, self.personal.parameters()),
            lr=LR, weight_decay=WEIGHT_DECAY,
        )
        self.lam = lam
    ...
    def get_parameters(self, config):
        # Only send GLOBAL model to server
    ...
    def set_parameters(self, params):
        ...
        # Restore personal model from persistent store
        # If first round for this client: initialize personal = global
        if self.client_id in _personal_models:
            self.personal.load_state_dict(_personal_models[self.client_id])
        else:
            # First round — initialize personal model from global weights
            self.personal.load_state_dict(copy.deepcopy(sd))
            # Reset personal optimizer since model changed
            self.opt_personal = optim.Adam(
                filter(lambda p: p.requires_grad, self.personal.parameters()),
                lr=LR, weight_decay=WEIGHT_DECAY,
            )
    ...
    def fit(self, parameters, config):
        ...
        lam          = config.get("lam", self.lam)
        local_epochs = config.get("local_epochs", LOCAL_EPOCHS)

        # ── Step 1: Update global model w (standard SGD, same as FedAvg) ──
        self.model.train()
        for _ in range(local_epochs):
            for images, labels in self.train_loader:
                images, labels = images.to(device), labels.to(device)
                self.opt_global.zero_grad()
                self.criterion(self.model(images), labels).backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)
                self.opt_global.step()

        # ── Step 2: Update personal model v_i toward updated w ─────────
        # Get current global weights as target (detached — no grad through w)
        global_params = [
            p.detach().clone()
        ...
        for _ in range(local_epochs):
            for images, labels in self.train_loader:
                images, labels = images.to(device), labels.to(device)
                self.opt_personal.zero_grad()

                # Task loss
                loss = self.criterion(self.personal(images), labels)

                # Proximal term: pull v_i toward w
                prox = torch.tensor(0.0, device=device)
                for v_param, w_param in zip(
                    (p for p in self.personal.parameters() if p.requires_grad),
                    global_params,
                ):
                    prox = prox + torch.sum((v_param - w_param) ** 2)

                loss = loss + (lam / 2.0) * prox
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.personal.parameters(), 1.0)
                self.opt_personal.step()

        # Save updated personal model back to persistent store
        _personal_models[self.client_id] = copy.deepcopy(
            self.personal.state_dict()
        )

        # Return GLOBAL model params to server for aggregation
        return self.get_parameters(config), len(self.train_loader.dataset), {}

    def evaluate(self, parameters, config):
        # Evaluate PERSONAL model — this is Ditto's key advantage
        self.set_parameters(parameters)
        acc, f1, auc, prec, rec, cm = evaluate_model(self.personal, self.test_loader)
...
# ─────────────────────────────────────────
def run_simulation(
    alpha: float, num_clients: int, seed: int, lam: float,
    max_rounds: int = NUM_ROUNDS,
    save_plots: bool = True,
) -> List[dict]:
    set_seed(seed)
    # Reset personal model store for this run — prevents bleedover between configs
    global _personal_models
    _personal_models = {}
    run_name = (ALGO + "_alpha" + str(alpha) + "_c" + str(num_clients)
                + "_lam" + str(lam) + "_seed" + str(seed))
    ...
    def client_fn(context: Context) -> fl.client.Client:
        return DittoClient(
            int(context.node_config["partition-id"]), data_dir, lam
        ).to_client()
    ...
    if save_plots:
        ...
        print("  Plot saved: ditto_plots/" + run_name + "_curves.png")
    return run_log

# ─────────────────────────────────────────
# Grid search for lambda
# ─────────────────────────────────────────
def tune_lambda(alpha: float, num_clients: int) -> float:
    """
    Grid search over lambda in {0.01, 0.1, 0.5, 1.0, 2.0}
    on 20-round trials. Returns best lambda by F1.
    """
    TUNE_ROUNDS  = 20
    LAMBDA_GRID  = [0.01, 0.1, 0.5, 1.0, 2.0]

    print("\n" + "=" * 58)
    print("  Ditto lambda Grid Search")
    print("  Config      : alpha=" + str(alpha) + "  clients=" + str(num_clients))
    print("  lambda grid : " + str(LAMBDA_GRID))
    print("  Rounds/trial: " + str(TUNE_ROUNDS))
    print("=" * 58 + "\n")

    results = []
    for i, lam in enumerate(LAMBDA_GRID):
        print("  [" + str(i+1) + "/" + str(len(LAMBDA_GRID))
              + "] lambda=" + str(lam))
        log      = run_simulation(
            alpha=alpha, num_clients=num_clients,
            seed=42, lam=lam,
        ...
        results.append({"lam": lam, "best_f1": best_f1, "best_acc": best_acc})
        print("  --> lambda=" + str(lam)
              + "  best F1=" + str(round(best_f1, 4))
    ...
    for r in results:
        marker = " <-- BEST" if r["lam"] == best_lam else ""
        print("    lambda=" + str(r["lam"])
    ...
    with open(os.path.join(RESULTS_DIR, "ditto_best_lambda.json"), "w") as f:
        json.dump(tune_results, f, indent=2)
    print("  Saved: ditto_results/ditto_best_lambda.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lam",         type=float, default=0.1,
                        help="Personalization weight lambda (default: 0.1)")
    ...
    parser.add_argument("--test",        action="store_true",
                        help="Quick 5-round test on alpha=1.0 c=5 to verify personal model learns")
    parser.add_argument("--run_tune",     action="store_true",
                        help="Grid search lambda first, then full runs")
    ...
    if args.test:
        print("\n" + "=" * 58)
        print("  DITTO SANITY TEST — 5 rounds, alpha=1.0, c=5")
        print("  Personal model should reach >50% by round 3")
        print("  If accuracy stays ~20-25%, personal model is broken")
        print("=" * 58 + "\n")
        log = run_simulation(
            alpha=1.0, num_clients=5, seed=42, lam=args.lam,
            max_rounds=5, save_plots=False,
        )
        print("\n  TEST RESULTS:")
        for r in log:
        ...
        if best > 50:
            print("\n  ✓ PASS — Personal model is learning (best: "
                  + str(round(best, 2)) + "%)")
        else:
            print("\n  ✗ FAIL — Personal model stuck at ~random ("
                  + str(round(best, 2)) + "%)")
            print("    Check _personal_models persistence and set_parameters()")
    ...
    if args.run_tune:
        ...
        print("  Using lambda=" + str(lam) + " for full runs\n")
    ...
    print("\n" + "=" * 62)
    print("  Ditto UATR — " + str(total) + " total runs")
    ...
    for alpha, num_clients in combos:
        ...
        for seed in args.seeds:
            ...
            print("  lambda=" + str(lam) + "  seed=" + str(seed)
                  + "  |  Log: " + log_path)
            print("  Time: " + time.strftime("%Y-%m-%d %H:%M:%S"))
            print("=" * 62 + "\n")

            t0      = time.time()
            log     = run_simulation(alpha, num_clients, seed, lam)
    ...
    pub.to_csv(os.path.join(RESULTS_DIR, "ditto_all_runs_summary.csv"), index=False)
    pub[["config", "accuracy_mean_std", "f1_mean_std", "n_seeds"]].to_csv(
        os.path.join(RESULTS_DIR, "ditto_publication_table.csv"), index=False
    )
    ...
    plot_combined(mean_logs, lam)
    plot_summary_bar(summary_rows, lam)
    ...
    print("\n  Results : ditto_results/")
    print("  Plots   : ditto_plots/")
    print("  Logs    : ditto_logs/")
    print("  Publication table: ditto_results/ditto_publication_table.csv")